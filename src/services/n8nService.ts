...
export const n8nService = {
      ...
      "nodes": [
        {
          ...
          "id": "2d335020-2d96-4d57-b8df-d390292d8483",
  ...
  // 🔧 Fonction pour corriger les workflows existants avec des problèmes
  async fixExistingWorkflow(workflowId: string): Promise<void> {
    console.log(`🔧 [n8nService] Correction du workflow existant ${workflowId}`);
    try {
      const workflow = await callN8nProxy('GET', `/workflows/${workflowId}`);
      
      let needsUpdate = false;
      const updatedNodes = workflow.nodes.map((node: any) => {
        // Corriger le Schedule Trigger
        if (node.type === 'n8n-nodes-base.scheduleTrigger') {
          const currentInterval = node.parameters?.rule?.interval?.[0];
      ...
      // Si on a fait des modifications, sauvegarder
      if (needsUpdate) {
        console.log('🔧 Sauvegarde du workflow corrigé...');
      ...
      console.error('❌ [n8nService] Erreur correction workflow:', error);
      throw error;
    }
  },
  ...
  async injectParams(workflowJson: any, params: Record<string, any>, userId: string, _userEmail?: string): Promise<any> {
    ...
    // 6. Créer le credential IMAP pour l'utilisateur
    ...
    if (params.USER_EMAIL && params.IMAP_PASSWORD && params.IMAP_SERVER) {
      try {
        ...
        console.error('❌ [injectParams] Erreur création credential IMAP:', error);
      }
    }
