// Déploiement spécifique pour "test mcp"
// Utilise directement mcpTestInjector et les fonctions utilitaires

const mcpTestInjector = require('../injectors/mcpTestInjector');
const db = require('../../database');
const deploymentUtils = require('./deploymentUtils');
const logger = require('../../utils/logger');

/**
 * Déploie le workflow "test mcp" avec sa logique spécifique
 */
async function deployWorkflow(template, credentials, userId, userEmail) {
  logger.info('Déploiement spécifique du workflow test mcp', {
  ...
  // 3. Injecter les credentials avec l'injecteur spécifique test mcp
  logger.debug('Injection des credentials avec mcpTestInjector', { templateId: template.id });
  ...
  logger.info('Workflow test mcp déployé avec succès', {
  ...
  return {
    success: true,
    message: 'Workflow test mcp déployé avec succès',
    workflow: {
      id: userWorkflow.id,
