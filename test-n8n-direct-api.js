...
// Test direct de l'API n8n (contourner le backend)
async function testN8nDirectAPI() {
  console.log('🔍 Test direct de l\'API n8n (contourner le backend)...');
  
  // Configuration n8n (à adapter selon votre config)
  const n8nUrl = 'http://localhost:5678'; // URL de votre instance n8n
  const n8nApiKey = 'n8n_api_key_here'; // Votre clé API n8n
  
  console.log('🔧 Configuration:');
  console.log('  - n8n URL:', n8nUrl);
  console.log('  - API Key:', n8nApiKey ? 'Présente' : 'Manquante');
  
  if (!n8nApiKey || n8nApiKey === 'n8n_api_key_here') {
    console.log('❌ Clé API n8n manquante. Veuillez configurer n8nApiKey dans le script.');
    return;
  }
...
// Menu principal
async function main() {
  console.log('🚀 Test direct de l\'API n8n\n');
  
  const args = process.argv.slice(2);
  
  if (args.length === 0) {
    console.log('📋 Options disponibles:');
    console.log('  node test-n8n-direct-api.js direct # Test direct de n8n');
    console.log('  node test-n8n-direct-api.js get    # Récupérer les credentials');
    console.log('  node test-n8n-direct-api.js all   # Tous les tests');
    console.log('');
    console.log('⚠️  IMPORTANT: Configurez n8nApiKey dans le script avant d\'utiliser "direct"');
    return;
  }
