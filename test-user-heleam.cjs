...
async function testUserHeleam() {
  try {
    console.log('🔍 Test avec user@heleam.com...');
    
    // 1. Connexion avec user@heleam.com
    console.log('1️⃣ Connexion user@heleam.com...');
    ...
    const loginReq = http.request(loginOptions, (loginRes) => {
      ...
      loginRes.on('end', () => {
        ...
        if (loginRes.statusCode === 200) {
          ...
          console.log('✅ Token user@heleam.com obtenu');
                  ...
                  const analyzeReq = http.request(analyzeOptions, (analyzeRes) => {
                    ...
                    analyzeRes.on('end', () => {
                      ...
                      if (analyzeRes.statusCode === 200) {
                        console.log('🎉 SUCCÈS ! L\'analyse fonctionne pour user@heleam.com !');
                        const result = JSON.parse(analyzeResponse);
                        console.log('- Workflow:', result.workflow);
                        console.log('- Credentials requis:', result.requiredCredentials?.length || 0);
                      } else if (analyzeRes.statusCode === 500) {
                        console.log('❌ Erreur 500 - Détails:', analyzeResponse);
                        console.log('🔍 Vérifiez les logs du backend pour voir l\'erreur exacte');
                      } else {
          ...
          console.log('❌ Échec connexion user@heleam.com');
